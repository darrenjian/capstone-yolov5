...
CLASS_MAPPING = {
    'LatAntHorn': 0,  # Lateral Anterior Horn -> tear
    'LatMenBody': 0,  # Lateral Meniscus Body -> tear
    'LatPosHorn': 0,  # Lateral Posterior Horn -> tear
    'MedAntHorn': 0,  # Medial Anterior Horn -> tear
    'MedMenBody': 0,  # Medial Meniscus Body -> tear
    'MedPosHorn': 0,  # Medial Posterior Horn -> tear
}
...
def create_dataset_yaml(output_root, stats_summary):
    ...
    yaml_content = f"""# YOLOv5 Dataset Configuration for Knee Meniscus Tear Detection (Single-Class)
...
# Note: 
# - Images WITH tears: labeled with bounding boxes (class 0)
# - Images WITHOUT tears: empty label files (background)
"""
...
def format_class_distribution(class_dist):
    ...
    # For single-class detection, aggregate all tear types
    total_tears = sum(count for class_name, count in class_dist.items()
                     if class_name in CLASS_MAPPING)
    if total_tears > 0:
        return f"#   0: tear - {total_tears} annotations"
